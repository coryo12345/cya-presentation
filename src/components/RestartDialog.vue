<template>
  ...
  <div v-if="show" class="fixed inset-0 bg-black/50 flex items-center justify-center z-50">
    <div class="bg-gray-800 rounded-lg shadow-xl p-6 max-w-sm w-full mx-4 text-white">
      <h2 class="text-xl font-semibold mb-4">Are you sure you want to restart?</h2>
      <div class="flex justify-end gap-3">
        <button @click="show = false" class="px-4 py-2 hover:bg-gray-700 rounded-md transition-colors cursor-pointer">
          No
        </button>
        <button
          @click="restart"
